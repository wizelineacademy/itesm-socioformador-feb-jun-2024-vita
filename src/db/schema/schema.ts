...
export const chronicalDesease = pgTable('ChronicalDesease', {
  ...
  idMedicalProfile: integer('id_medical_profile')
    .notNull()
    .references(() => medicalProfile.idMedicalProfile, {
      onDelete: 'cascade',
      onUpdate: 'cascade',
    }),
...
export const goalEvaluation = pgTable('GoalEvaluation', {
  ...
  idGoal: integer('id_goal')
    .notNull()
    .references(() => Goals.idGoal, {
      onDelete: 'cascade',
      onUpdate: 'cascade',
    }),
  name: varchar('name', { length: 50 }).notNull(),
...
export const badges = pgTable('Badges', {
  ...
  idChallenge: integer('id_challenge').references(
    () => monthlyChallenge.idChallenge,
...
export const userBadges = pgTable(
  'UserBadges',
  {
    ...
    badgeId: integer('badge_id').references(() => badges.idBadge, {
      onDelete: 'cascade',
      onUpdate: 'cascade',
    }),
  },
...
export const challengeEvaluations = pgTable(
  'ChallengeEvaluations',
  {
    ...
    idUser: integer('id_user')
      ...
      .notNull(),
    idChallenge: integer('id_challenge')
      .references(() => monthlyChallenge.idChallenge, {
        onDelete: 'cascade',
        onUpdate: 'cascade',
      })
      .notNull(),
  ...
  (table) => {
    return {
      unqEvaluation: unique('unique_evaluation').on(
        table.idUser,
        table.idChallenge,
        table.evaluatorId,
      ),
