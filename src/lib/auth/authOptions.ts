...
export const authOptions: NextAuthOptions = {
    ...
    CredentialsProvider({
      ...
      authorize: async (credentials, req) => {
        ...
        if (existingUser.length === 0) {
          throw new Error(
            JSON.stringify({
              errors: 'El correo no se encuentra registrado',
              status: false,
            }),
          )
        }
        ...
        if (!existingUser[0].password) {
          throw new Error(
            JSON.stringify({
              errors:
                'Este correo se encuentra registrado con Google o Facebook',
              status: false,
            }),
          )
        }
        ...
        throw new Error(
          JSON.stringify({
            errors: 'El correo y contraseña son incorrectos',
            status: false,
          }),
        )
      },
  ...
  callbacks: {
    async signIn({ account, profile }) {
      //register users if they do not exist in the database
