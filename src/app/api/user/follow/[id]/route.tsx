...
export async function POST(
  ...
  try {
    ...
    const existingFollow = await db
      ...
      .where(
        and(
          eq(following.userId, followerId),
          eq(following.followingId, followedId),
        ),
      )
      .limit(1)
    ...
    if (existingFollow.length > 0) {
      ...
      await db
        ...
        .where(
          and(
            eq(followers.userId, followedId),
    ...
    console.error(error)
    return NextResponse.json(
      { message: 'Error handling follow/unfollow' },
      { status: 500 },
    )
  }
}
    ...
    const followStatus = await db
      ...
      .where(
        and(
          ...
          eq(following.followingId, requestedUserId),
        ),
      )
      .limit(1)
