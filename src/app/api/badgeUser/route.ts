 ...
 */
export async function addUserPointsAndBadges(
  userId: number,
  pointsToAdd: number,
  badgeId: number,
) {
  try {
    ...
    if (existingBadge.length > 0) {
      return NextResponse.json('The table userBadges has already been filled', {
        status: 409,
      })
    }
    ...
    const userPointsEntry = await db
      .select()
      .from(userPoints)
    ...
    return {
      success: true,
      message: 'User points and badges added successfully',
    }
  } catch (error) {
