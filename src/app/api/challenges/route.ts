...
const getUsersWithNumber = async () => {
  const users = await db.select().from(user).where(isNotNull(user.phoneNumber))
  return users
}

const sendMessageWp = async (challenge: {
  name: string
  description: string
}) => {
  try {
    ...
    const promises = users.map((user) =>
      axios.post(
        `https://graph.facebook.com/v18.0/248211301718489/messages`,
        {
          ...
          text: {
            body: `Hay un nuevo desafío disponible:\n${challenge.name}\n${challenge.description}`,
          },
        },
        {
          headers: {
...
export async function POST(request: Request) {
  try {
    ...
    if (existingChallenge.length > 0) {
      ...
      return NextResponse.json('Ya existe un reto para este mes y año', {
        status: 409,
      })
    }
    ...
    return NextResponse.json(
      { message: 'Reto creado exitosamente', data: newChallenge },
      { status: 201 },
    )
  } catch (error) {
...
export async function GET() {
  try {
    ...
    return NextResponse.json('Error fetching monthly challenge', {
      status: 500,
    })
  }
}

async function checkOrCreateBadge(
  name: string,
  description: string,
  idChallenge: number,
): Promise<void> {
  try {
    ...
    if (existingBadge.length === 0) {
      ...
      await db
        .insert(badges)
        .values({ idChallenge: idChallenge, name, description })
