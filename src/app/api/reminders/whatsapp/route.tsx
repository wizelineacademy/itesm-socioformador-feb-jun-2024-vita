...
import { Reminders, user } from '@/src/db/schema/schema'
import { addSecondsToDate } from '@/src/lib/DaysFormat/days'
...
async function getOverdueReminders() {
  ...
  const res = await db
    ...
    .where(
      and(isNotNull(Reminders.dueTime), lte(Reminders.dueTime, currentDate)),
    )
...
async function updateDueTimeReminders(idsReminders: number[]) {
  ...
  const updatedReminders = remindersToUpdate.map((reminder) => {
    let newDueTime: Date | null = addSecondsToDate(
      reminder.dueTime,
      reminder.frequency,
    )
    //if the next due time is past the end time, set it to null since the message will stop being sent
    newDueTime =
      reminder.endTime && newDueTime > reminder.endTime ? null : newDueTime
    return {
      ...reminder,
      dueTime: newDueTime,
    }
