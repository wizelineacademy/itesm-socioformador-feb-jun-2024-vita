...
const GoalsDetailPage = ({ params }: { params: { idGoal: string } }) => {
  ...
  const validateGoalNumeric = (): boolean => {
    ...
    if ((goal as NumericGoal).constraint === 'increase' && next <= previous) {
      ...
      (goal as NumericGoal).constraint === 'decrease' &&
      next >= previous
    ) {
      Swal.fire({
        title: 'Error',
          ...
          >
            {isNumericGoal(goal) && (
              <>
                <LbMsrInput
                  ...
                  label={
                    goal.id === 1
                      ...
                      : `¿En qué ${goal.variable} te encuentras?`
                  }
                  ...
                  setValue={
                    setPrevious as React.Dispatch<React.SetStateAction<number>>
                  }
                />
                ...
                <LbMsrInput
                  ...
                  label={
                    goal.id === 1
                      ? '¿Cuántas veces a la semana deseas hacer ejercicio?'
                      : `¿Qué ${goal.variable} te gustaría alcanzar?`
                  }
                  variable={goal.variable ?? ''}
                  min={(goal as NumericGoal).min ?? 0}
                  max={(goal as NumericGoal).max ?? 0}
                  ...
                  setValue={
                    setNext as React.Dispatch<React.SetStateAction<number>>
                  }
                />
              </>
            )}
            ...
            {isCategoricalGoal(goal) && (
              <LbSelect
                ...
                setValue={
                  setNext as React.Dispatch<React.SetStateAction<string>>
                }
              />
            )}
