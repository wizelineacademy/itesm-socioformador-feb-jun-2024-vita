...
import { UserPost, Follower} from "@/src/data/datatypes/user"; 
import axios from "axios";
import { useEffect, useState } from "react";

const ProfilePosts = ({ params }: { params: { id: string } }) => {
  const id = params.id;
  const [userFollowers, setUserFollowers] = useState<Follower[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      await Promise.all([getUserFollowers(), getUser(), getUserProfile()]);
      setLoading(false);
    };
