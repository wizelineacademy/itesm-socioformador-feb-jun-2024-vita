...
const NutritionalPlan = () => {
  ...
  const orderRecipes = (recipes: Recipe[]) => {
    const desayuno = recipes.filter(recipe => recipe.mealtime?.toLocaleLowerCase() === "desayuno")
    const comida = recipes.filter(recipe => recipe.mealtime?.toLocaleLowerCase() === "comida")
    const snack = recipes.filter(recipe => recipe.mealtime?.toLocaleLowerCase() === "snack")
    const cena = recipes.filter(recipe => recipe.mealtime?.toLocaleLowerCase() === "cena")
  ...
  return (
    <>
      <div className="ml-5 mr-5">
          <h2 className={"mt-2 text-4xl text-white font-semibold md:mt-10"}>Mi plan nutricional</h2>
