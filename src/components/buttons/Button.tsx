...
const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`font-notosans text-md relative w-40 rounded-full py-3 font-bold text-white transition hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-70 sm:w-60 sm:text-sm ${outline ? borderColor : ''} ${big ? 'text-xl' : 'text-sm'} ${big ? 'w-60' : 'w-60'} // Cambiarlo mas adelante no se el tamano del boton mediano ${big ? 'border-[3px]' : 'border-[3px]'} // Cambiarlo mas adelante no se el tamano del borde`}
      style={{
        background: 'none',
      }}
    >
      {Icon && <Icon size={24} className='absolute left-2 top-3' />}
      {label}
    </button>
  )
}
