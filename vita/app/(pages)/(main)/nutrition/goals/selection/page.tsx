import { nutritionGoals } from "@/data/nutrition_goals";
import Link from "next/link";

const GoalsSelectionPage = () => {

    return (
        <div className="bg-[#2C0521] p-4 text-white flex flex-col items-start justify-start space-y-4 pt-10 md:items-start">
            <h2 className="text-5xl font-bold mb-4">Mi Meta</h2>
            <p className="text-xl font-bold">Selecciona una meta de ejercicio</p>
            <div className="pt-3 w-full mx-auto flex flex-col justify-center gap-y-8">
                {nutritionGoals.map(goal => (
                    <Link key={goal.title} href={`/nutrition/goals/selection/${goal.id}`}>
                        <p 
                            className="w-full sm:w-4/5 md:max-w-[600px] py-5 px-2 md:py-6 leading-5 rounded-full font-bold text-center sm:pl-10 sm:text-left bg-decoration-nutrition-colorlight hover:bg-decoration-nutrition-colordark hover:cursor-pointer" 
                        >
                            {goal.title}
                        </p>
                    </Link>
                ))}
            </div>
        </div>

    )
}

export default GoalsSelectionPage;